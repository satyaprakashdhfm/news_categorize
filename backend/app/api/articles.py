...
from app.schemas import (
    ArticleResponse, ArticleListResponse, StatsResponse,
    CountryCount, CategoryCount, SUBCATEGORY_LABELS,
)
import logging
...
async def get_articles(
    ...
    try:
        ...
        if country:
            query = query.filter(Article.country == country.upper())

        if categories:
            category_list = [cat.strip().upper() for cat in categories.split(',') if cat.strip()]
            if category_list:
                query = query.filter(Article.category.in_(category_list))

        if subcategory:
            query = query.filter(Article.subcategory == subcategory.upper())
        ...
        if stats:
            ...
            recent_articles = db.query(Article).filter(Article.scraped_at >= yesterday).count()
            active_threads = db.query(Article.thread_id).filter(
                Article.thread_id.isnot(None)
            ).distinct().count()

            country_counts_raw = db.query(
                Article.country, func.count(Article.country).label('count')
            ).filter(Article.country.isnot(None)).group_by(Article.country).all()
            country_counts = [CountryCount(country=c[0], count=c[1]) for c in country_counts_raw]

            category_counts_raw = db.query(
                Article.category, func.count(Article.category).label('count')
            ).group_by(Article.category).all()
            category_counts = [CategoryCount(category=str(c[0].value), count=c[1]) for c in category_counts_raw]
...
async def get_subcategory_map():
    """Return subcategory codes and labels for frontend use."""
    return SUBCATEGORY_LABELS
